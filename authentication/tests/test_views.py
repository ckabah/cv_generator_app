...
class AuthTest(TestCase):
    ...
    def test_register_view_post(self):
        response = self.client.post(
            ...
            data={
                ...
                "password1": "vkjoierjio7",
                "password2": "vkjoierjio7",
            },
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_login_view(self):
        response = self.client.post(
            reverse("login"),
            data={"email": "cs.ttrx@gmail.com", "password": "coull1515fdf"},
        )
        self.assertEqual(response.status_code, 404)  # will for account acctivation
        response = self.client.post(
            reverse("login"),
            data={"email": "coul@gmail.com", "password": "godejeroien583"},
        )
        self.assertEqual(
            response.status_code, 302
        )  # superuser have a active account, so test will be successfully
        self.assertEqual(response.url, reverse("dashbord"))  # logged user name
        login = self.client.login(
            user_name="coul@gmail.com", password="godejeroien583"
        )  # user name field is user_name
        self.assertTrue(login)

    def test_password_reset_viwew(self):
        response = self.client.post(
            reverse("password_reset"), data={"email": "coul@gmail.com"}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_password_reset_confirm_view(self):
        ...
        response = self.client.post(
            ...
            data={"new_password1": "tejzoiej57", "new_password2": "tejzoiej57"},
        )
