...
class TestCvView(TestCase):
    def setUpTestData(cls):
        ...
        Profile.objects.create(
            ...
            last_name="Soumaila",
            bio="Coulibaly Soumaila bio",
        )
        Education.objects.create(
            user=user,
            name="AI and Multi-media",
            etablisement="University of Batna2",
            description="Ai and Multi-media",
        )
        ProfExperience.objects.create(
            user=user,
            type="stage",
            description="Machine engeneer stage",
            company="google",
        )

        Project.objects.create(
            user=user,
            name="Laon prediction using ML",
    ...
    def test_edit_view(self):
        ...
        self.assertEqual(response.status_code, 200)
        response = self.client.post(
            reverse("edit_experience", args=[1]),
            data={
                "type": "Stage",
                "company": "facebook",
                "description": "expencience description",
    ...
    def test_delete_view(self):
        ...
        self.assertTrue(login)
        response = self.client.post(
            reverse("add_langue"), data={"name": "English", "level": 90}
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.status_code, 200)
